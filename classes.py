...
class board(object):
    ...
    def check_win(self, coord):
        ...
        for w in self.coord_win[coord]:
            try:
                # Editing wins dictionary:
                self.wins[player][w] += 1
                self.score = self.score + 1 if player == 1 else self.score - 1 #Updating score
                # Detecting Win
    ...
    def move_b(self, column): # Useful repeatable code for bot movement
        current = copy(self)
...
class tree(): # tree to make the minimax traversal easier
    def __init__(self, origin, bd, minmax, column):
        self.board: board
        ...
        self.children = []
        self.col = column
        self.score_board_col = None
    
    def add_children(self, child): # Add a child to the tree
        self.children.append(child)
    
    def add_score(self, score, board, player, col): #Editing max/min score attributed to the subtree
        if self.score == None:
            self.score = score
            self.score_board = board
            self.score_board_col = col
        elif player == 1:
...
class game():
    ...
    def move_bot(self, DEPTH): #Minmax algorithm to determine best move
        ...
        # Creating tree
        current = copy(self.board)
        tr = tree(None, current, True, None)
        parent = tr
        ls = [tr]
            ...
            for bds in ls:
                ...
                if bds.board.won:
                    ...
                    for column in bds.board.check_possible_moves():
                        ...
                        child = tree(parent, result, i%2 == 0, column)
                        if i == 1 and result.won:
                            self.board = result
                            return column
                        if i != 2:
                            # Edge cases to prevent stupid game-ending mistakes from the AI
                            if edge_case_check is False:
                                ...
                                if self.board.state[-1][3] == 1 and self.board.state[-1][2] == 1 and self.board.state[-1][1] == None and self.board.state[-1][4] == None:
                                    self.move(1)
                                    return 1
                                if self.board.state[-1][2] == 1 and self.board.state[-1][4] == 1 and self.board.state[-1][3] == None and (self.board.state[-1][1] == None or self.board.state[-1][5] == None):
                                    self.move(3)
                                    return 3
                                if self.board.state[-1][3] == 1 and self.board.state[-1][5] == 1 and self.board.state[-1][4] == None and self.board.state[-1][6] == None and self.board.state[-1][2] == None:
                                    self.move(4)
                                    return 4
                                if self.board.state[-1][1] == 1 and self.board.state[-1][3] == 1 and self.board.state[-1][2] == None and self.board.state[-1][0] == None and self.board.state[-1][4] == None:
                                    self.move(2)
                                    return 2
        ...
        if not ls:
            ...
            return current.check_possible_moves()[0]
        while ls[0].origin != None:
            ...
            for child in ls:
                ...
                if child.origin == None:
                    ...
                    return child.score_board_col
                child.origin.add_score(child.score, child.board, self.board.player, child.col)
            ls = new_ls
        # Updating game
        self.board = tr.score_board
        if self.board.won:
            exit()
        return tr.score_board_col
